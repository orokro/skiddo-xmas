...
// --- 2. RAW SONG DATA (Cosmic Music Box) ---
const sourceData = {
  "tracks": [
    {
      "id": 1, "notes": [
...
function mapPitchToKeyIndex(midi) {
    ...
    // Simple safeguard to prevent infinite loops, though unlikely here
    let safeguard = 0;
...
sourceData.tracks.forEach(track => {
    track.notes.forEach(n => {
        ...
        // Round time to 2 decimals to catch micro-overlaps
        ...
        if (!seen.has(id)) {
            ...
            processedNotes.push({
                startTime: n.time,
                endTime: n.time + 1.0, // Standard sustain
                key: k
            });
        }
    });
